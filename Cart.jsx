...
function Cart({ cart, changeQuantity, removeItem, salesTaxRate }) {
  const subTotal = () => {
    let totalSubTotal = 0;
    cart.forEach((item) => {
      totalSubTotal += +item.quantity * +(item.salePrice || item.originalPrice);
    });
    return totalSubTotal;
  };

  let totalTaxes = Math.floor(subTotal() * salesTaxRate * 100) / 100;
